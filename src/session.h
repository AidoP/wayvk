...
typedef void (*fn_session_update)(void* data, struct session*, Vulkan*, Font*);
...
static inline void session_update(Vulkan* vk, Font* ft, SessionHandler* handler) {
    handler->update(handler->data, &handler->session, vk, ft);
}